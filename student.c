...
int accountExists(const char* studentID) {
    for (int i = 0; i < studentCount; i++) {
        if (strcmp(student[i].id, studentID) == 0) return 1;
    }
    return 0;
}
...
int studentLogin(void) {
    ...
    int index = -1;
    for (int i = 0; i < studentCount; i++) {
        if (strcmp(student[i].id, studentID) == 0) { index = i; break; }
    }
    if (index == -1) {
    ...
    if (strcmp(student[index].password, password) == 0) {
...
void changePassword(void) {
    ...
    if (strcmp(student[index].password, oldPassword) != 0) {
        printf("\t\tIncorrect old password.\n");
        return;
    }

    inputPassword(newPassword);
    strcpy(student[index].password, newPassword);
...
Student_std* findStudent(const char* studentID) {
    for (int i = 0; i < studentCount; i++) {
        if (strcmp(student[i].id, studentID) == 0) {
            return &student[i];
        }
    }
    return NULL;
